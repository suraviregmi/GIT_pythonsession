...
def init_db():
    ...
    cur = db_conn.cursor()
    _sql='''SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'peoples' '''
    cur.execute(_sql)
    if not cur.fetchone():
        _create_sql = '''CREATE TABLE peoples(id INTEGER PRIMARY KEY AUTOINCREMENT, 
...
def index():
    ...
    open http://localhost5000 on browser
    """
...
def add_people():
    ...
    if request.method == 'POST':
        db_conn = get_connection()
        cur = db_conn.cursor()
        # save data to database and redirect to list page
        ...
        if firstname.strip():
            #save to db
            _sql = '''INSERT INTO  peoples (firstname, lastname, address, country) VALUES(?, ?, ?, ?)'''
            cur.execute(_sql,(firstname.strip(), lastname.strip(), address.strip(), country.strip() ))
...
def list_people():
    ...
    db_conn = get_connection()
    cur = db_conn.cursor()
    _sql = "SELECT id, firstname, lastname, address, country FROM  peoples"
    cur.execute(_sql)
